...
def calculate_grade(get_IP_vector,
                    init_permute,
                    get_FP_vector,
                    final_permutation,
                    get_E_vector,
    ...
    if_get_IP_vector = test_get_IP_vector(get_IP_vector)
    if_init_permute = test_init_permute(init_permute)
    if_get_FP_vector = test_get_FP_vector(get_FP_vector)
    if_final_permutation = test_final_permutation(final_permutation)
    if_get_E_vector = test_get_E_vector(get_E_vector)
    if_expansion_function = test_expansion_function(expansion_function)
    if_get_S_box = test_get_S_box(get_S_box)
    if_s_box_calculation = test_s_box_calculation(s_box_calculation)
    if_get_P_box = test_get_P_box(get_P_box)
    if_permutation = test_permutation(permutation)
    if_round_function = test_round_function(round_function)
    if_get_PC1_vector = test_get_PC1_vector(get_PC1_vector)
    if_permutation_choice_1 = test_permutation_choice_1(permutation_choice_1)
    if_get_key_rotation = test_get_key_rotation(get_key_rotation)
    if_circular_left = test_circular_left(circular_left)
    if_get_PC2_vector = test_get_PC2_vector(get_PC2_vector)
    if_compression_pbox = test_compression_pbox(compression_pbox)
    if_key_generation = test_key_generation(key_generation)
    if_feistel_round = test_feistel_round(feistel_round)
    if_des_cipher = test_des_cipher(des_cipher)
    if_des_app = test_des_app(des_app)
    
    grade_summary = [
        ['Index','Exercise', 'Passed', 'Grade'],
        ['1','get_IP_vector()',str(if_get_IP_vector),str(if_get_IP_vector * 2)],
        ['2','init_permute()',str(if_init_permute), str(if_init_permute * 5)],
        ['3','get_FP_vector()',str(if_get_FP_vector), str(if_get_FP_vector * 2)],
        ['4','final_permutation()',str(if_final_permutation),str(if_final_permutation * 5)],
        ['5','get_E_vector()',str(if_get_E_vector),str(if_get_E_vector * 2)],
        ['6','expansion_function()',str(if_expansion_function), str(if_expansion_function * 5)],
        ['7','get_S_box()',str(if_get_S_box),str(if_get_S_box * 2)],
        ['8','s_box_calculation()',str(if_s_box_calculation),str(if_s_box_calculation * 8)],
        ['9','get_P_box()',str(if_get_P_box),str(if_get_P_box * 2)],
        ['10','permutation()',str(if_permutation),str(if_permutation * 5)],
        ['11','round_function()',str(if_round_function),str(if_round_function * 5)],
        ['12','get_PC1_vector()',str(if_get_PC1_vector),str(if_get_PC1_vector * 2)],
        ['13','permutation_choice_1()',str(if_permutation_choice_1),str(if_permutation_choice_1 * 5)],
        ['14','get_key_rotation()',str(if_get_key_rotation),str(if_get_key_rotation * 2)],
        ['15','circular_left()',str(if_circular_left), str(if_circular_left * 5)],
        ['16','get_PC2_vector()',str(if_get_PC2_vector),str(if_get_PC2_vector * 2)],
        ['17','compression_pbox()',str(if_compression_pbox),str(if_compression_pbox * 5)],
        ['18','key_generation()',str(if_key_generation),str(if_key_generation * 10)],
        ['19','feistel_round()',str(if_feistel_round), str(if_feistel_round * 6)],
        ['20','des_cipher()',str(if_des_cipher), str(if_des_cipher * 10)],
        ['21','des_app()',str(if_des_app), str(if_des_app * 10)]
    ]
    
    exam_grade = 0
    for i in range(21):
        exam_grade += int(grade_summary[i+1][3])
        

    grade_table = AsciiTable(grade_summary)
    print(grade_table.table)
    print("Exam Grade: ", exam_grade)
